...
def cart(request):
    ...
    if not request.user.profile.is_player:
        messages.warning(request, "Apenas perfis de jogadores podem comprar na loja. Faça o login / registre-se como um jogador primeiro.")
    ...
    if request.method == 'GET':
        ...
        if not user_is_player(request.user):
            jsondata['error'] = "Você não é um jogador, assim você não pode comprar nenhum jogo."
        ...
        if form.cleaned_data['action'] == 'add':
            ...
            if owned_games.count() > 0:
                jsondata['error'] = "Você já possui este jogo"
            ...
            if form.cleaned_data['game'].id in curcart:
                jsondata['error'] = "O carrinho já contém este jogo"
    ...
    if request.method == 'GET':
        ...
        for g in qset:
            if request.user.profile._ownedGames.filter(id__exact=g.id).count() > 0:
                messages.error(request,"Você já é dono do jogo %s. Este item foi removido do carrinho"
                                 % g.name)
        ...
        if qset.count() < 1:
            messages.warning(request, "O carrinho esta vazio!")
            return HttpResponseRedirect(reverse("cart"))
        ...
        messages.info(request, "Ordem criada com sucesso.")
...
def order_details(request, order_id):
    if request.method == 'GET':
        ...
        if order._player != request.user.profile:
            messages.error(request, "Você não tem permissão para gerenciar este pedido")
            return HttpResponseRedirect(reverse("cart"))

        # Is the associated pid in a correct status?
        if order.status != "pending" and order.status != "error":
            messages.error(request, "A ordem especificada não está em um status válido para ser processada novamente.")
            return HttpResponseRedirect(reverse("cart"))

        # Ok, everything seems fine. Print again the submit form.
        action = "https://simplepayments.herokuapp.com/pay/"  # Fixme Todo: parametrize
        amount = order.total
        sid = "pow2019"  # Fixme Todo: parametrize
        ...
        secret_key = "4b09ec9bd3d650cd52216ea9cdbce5b9"  # Fixme Todo: parametrize
...
def payment_result(request):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            messages.error(request, "ID do pedido inválido especificado. Seu pagamento é inválido.")
            return HttpResponseRedirect(redirect_to=reverse("cart"))

        # Third: is this user allowed to manipulate this order?
        if order._player != request.user.profile:
            messages.error(request, "O ID do pedido especificado não é seu. Seu pagamento é inválido.")
            return HttpResponseRedirect(redirect_to=reverse("cart"))

        # Fourth: is the order status consistent?
        if order.status != "pending":
            messages.error(request, "O ID do pedido especificado não está no status pendente. Por favor, faça outro pedido.")
            return HttpResponseRedirect(redirect_to=reverse("cart"))
        ...
        if result == "success":
            ...
            order.delete()
            messages.info(request,"O pedido foi cancelado conforme solicitado.")
            return HttpResponseRedirect(redirect_to=reverse("cart"))
...
def thankyou(request):
    messages.success(request, "Obrigado pela sua compra. Aproveite o novo jogo!")
...
def developer_games(request):
    ...
    if request.method == 'GET':
        ...
        if not form.is_valid():
            ...
            messages.success(request=request, message='O jogo foi adicionado com sucesso!')
...
def edit_game(request, game_id):
    if request.method == 'GET':
        ...
        if not form.is_valid():
            messages.error(request, "Por favor, verifique os erros do formulário e tente novamente.")
            return render(request, 'devzone/edit_game.html', {'form': form, 'user': request.user})
        else:
            ...
            if request.POST['action'].lower() == 'save':
                game = form.save()
                messages.success(request=request, message='Jogo atualizado com sucesso.')
                return HttpResponseRedirect(redirect_to=reverse('dev_games'))
            elif request.POST['action'].lower() == 'delete':
                game.delete()
                messages.success(request=request, message='Jogo removido com sucesso.')
                return HttpResponseRedirect(redirect_to=reverse('dev_games'))
            else:
                messages.error(request=request, message='Missing or invalid action parameter.')

...
def games(request):
    ...
    if request.method == "GET":
        ...
        if orderby not in ['popularity', 'name', 'category', 'price']:
            return json_400("Ordem com parametro invalido.")
        if cat is not None:
            try:
                cat = int(cat)
            except (ValueError, TypeError):
                return json_400("Erro na categoria talvez seja um inteiro")
        try:
            page = int(page)
        except (ValueError, TypeError):
            return json_400("Erro no parametro, talve seja um inteiro.")
...
def game(request, game_id):
    ...
    if request.method == "GET":
        try:
            ...
            return json_404("Jogo não existe")
...
def category(request, category_id):
    ...
    if request.method == "GET":
        try:
            ...
            return json_404("Categoria não existe.")
